...
@pytest.fixture
def typ():
    t = ReleaseType(name='Release Type')
    t.save()
    return t


@pytest.mark.django_db
def test_source_objects_api():
    t = typ()

    # Create a translatable object
    t1 = Release(release_type=t, title='Science Release', id='eso1501')
    t1.save()
    ...
    t2 = Release(release_type=t, title='Organisation Release', lang=settings.LANGUAGE_CODE, id='eso1502')
    t2.save()
    ...
    t3 = Release(release_type=t, title='Pressemeddelelse', lang='da', id='eso1502da')
    t3.save()
    assert t3.lang == 'da'
...
@pytest.mark.django_db
def test_translated_objects_api():
    t = typ()

    # Create a source object
    t1 = Release(release_type=t, title='Science Release', published=True, release_date=datetime(2010, 3, 5, 12, 36), id='eso1501')
    t1.save()
...
@pytest.mark.django_db
def test_translation_objects_filters():
    t = typ()

    # Setup
    t1 = Release(release_type=t, title='Science Release', published=False, id='eso1501')
    t2 = Release(release_type=t, title='Photo Release', published=True, id='eso1502')
    t3 = Release(release_type=t, title='Organisation Release', published=True, id='eso1503')
    t4 = Release(release_type=t, title='Hallo', published=True, lang='da', id='eso1504')
    _ = [x.save() for x in [t1, t2, t3, t4]]
    t1_da = Release(release_type=t, title='Pressemeddelelse (Videnskabelig)', published=True, source=t1, lang='da', id='eso1501da')
    t2_da = Release(release_type=t, title='Pressemeddelelse (Billed)', published=False, source=t2, lang='da', id='eso1502da')
    t3_da = Release(release_type=t, title='Pressemeddelelse (Organisation)', published=False, source=t3, lang='da', id='eso1503da')
    t4_en = Release(release_type=t, title='Hello', published=False, source=t4, lang='en', id='eso1504en')
...
def test_translations():
    ...
    # Setup
    t1 = Release(release_type=t, title='Press Release', published=True, id='eso1501')
    t2 = Release(release_type=t, title='Test 1', published=True, id='eso1502')
    t3 = Release(release_type=t, title='Test 2', published=True, lang='da', id='eso1503')
    _ = [x.save() for x in [t1, t2, t3]]
    t1_da = Release(release_type=t, title='Pressemeddelelse', published=True, source=t1, lang='da', id='eso1501da')
    t1_de = Release(release_type=t, title='Pressemitteilung', published=True, source=t1, lang='de', id='eso1501de')
...
@pytest.mark.django_db
def test_source_updates():
    t = typ()

    t1 = Release(release_type=t, title='Press Release', published=True, release_date=datetime(2010, 3, 5, 12, 00), id='eso1501')
    t2 = Release(release_type=t, title='Test 1', published=True, release_date=datetime(2010, 4, 5, 12, 00), id='eso1502')
    t1.save()
    t2.save()
...
@pytest.mark.django_db
def test_related_translations():
    t = typ()

    # Setup
    t1 = Release(release_type=t, title='Item 1', id='eso1501')
    t2 = Release(release_type=t, title='Item 2', id='eso1502')
    t3 = Release(release_type=t, title='Item 3', id='eso1503')
    _ = [x.save() for x in [t1, t2, t3]]
...
@pytest.mark.django_db
def test_non_en_source():
    t = typ()

    # Setup
    t1 = Release(release_type=t, title='Pressemitteilung', published=True, lang='de', id='eso1501de', created=datetime.now())
    t1_en = Release(release_type=t, title='Press Release', published=True, source=t1, lang='en', id='eso1501de-en', created=datetime.now())
